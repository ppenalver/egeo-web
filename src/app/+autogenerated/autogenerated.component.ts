   ...
   private onResponse(doc: Documentation): void {
      ...
      doc.api.outputs.forEach(ouput => {
         this.apiDoc.apiSection.outputs.push({
            ...
            required: ouput.required,
            details: _unescape(ouput.description + (ouput.defaultValue && ouput.defaultValue.length > 0 ? `, DEFAULT=${ouput.defaultValue}` : ''))
         });
      });
   }
}
